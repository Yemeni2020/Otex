        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 space-y-8">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="text-slate-600 mt-2">Issued on {{ \Carbon\Carbon::parse($placedAt)->toFormattedDateString() }}</p>
                </div>
                    ...
                    <tbody class="divide-y divide-slate-200 text-sm">
                        @forelse ($items as $item)
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3 text-right font-semibold text-slate-900">${{ number_format($price * $qty, 2) }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td class="px-4 py-3 text-slate-600" colspan="4">No items found for this invoice.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div class="bg-slate-50 border border-slate-200 rounded-xl p-4">
                    ...
                    <p class="text-sm text-slate-800">{{ $payment['brand'] ?? 'Card' }} **** {{ $payment['last4'] ?? '0000' }}</p>
