...
<head>
    ...
    <style>
        ...
        .nav-bounce {
            animation: navBounce 420ms ease;
        }
...
<body class="bg-gray-100 text-gray-900 min-h-screen expansion-alids-init">
    ...
    <!-- Scroll to top -->
    <button id="scrollToTop" id="scrollToTopPath" aria-label="Scroll to top"
        class="scroll-to-top fixed bottom-6 right-4 z-35 hidden sm:block transition-opacity duration-300 opacity-0"
        type="button">
    ...
    <script>
        ...
        (() => {
            ...
            const setHeaderOffset = () => {
                const topbarHeight = topbar && !topbar.classList.contains('topbar-hidden') ? topbar.offsetHeight :
                    0;
                const headerHeight = header ? header.offsetHeight : 0;
                document.documentElement.style.setProperty('--topbar-height', `${topbarHeight}px`);
                document.documentElement.style.setProperty('--header-stack-height',
        ...
        `;
            ...
            body.querySelector('button').addEventListener('click', remove, {
                once: true
            });
