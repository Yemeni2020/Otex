        ...
        <section class="relative mt-6">
            ...
            <div id="newArrivalsViewport"
                ...
                @foreach($this->visibleProducts as $product)
                    <article class="slide snap-start shrink-0 w-[220px] sm:w-[240px] md:w-[260px]" wire:key="new-arrival-{{ $product['id'] }}">
                        <div class="group rounded-2xl border border-zinc-200 bg-white shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                            <div class="relative aspect-[4/3] bg-zinc-100 overflow-hidden">
                                ...
                                <div class="absolute left-3 top-3">
                                    @if(!empty($product['badge']))
                                        <span class="rounded-full bg-white/90 px-2.5 py-1 text-[11px] font-semibold text-zinc-800 shadow-sm ring-1 ring-black/5">{{ $product['badge'] }}</span>
                                    @else
                                        <span class="rounded-full bg-white/90 px-2.5 py-1 text-[11px] font-semibold text-zinc-800 shadow-sm ring-1 ring-black/5">{{ $product['category'] }}</span>
                                    @endif
                                </div>
                            ...
                            <div class="p-4">
                                ...
                                <h3 class="mt-1 text-sm font-bold text-zinc-900 line-clamp-2">{{ $product['name'] }}</h3>
                                @if(isset($product['rating'], $product['reviews']))
                                    <div class="mt-2 flex items-center gap-2">
                                        <div class="flex items-center gap-0.5 text-amber-500" aria-hidden="true">
                                            @for ($i = 0; $i < 5; $i++)
                                                <svg viewBox="0 0 24 24" class="h-4 w-4" fill="currentColor" opacity="{{ $i < floor($product['rating']) ? '1' : '0.35' }}">
                                                    <path d="M12 17.3l-6.18 3.73 1.64-7.03L2 9.24l7.19-.61L12 2l2.81 6.63 7.19.61-5.46 4.76 1.64 7.03z"/>
                                                </svg>
                                            @endfor
                                        </div>
                                        <span class="text-xs font-semibold text-zinc-700">{{ number_format($product['rating'], 1) }}</span>
                                        <span class="text-xs text-zinc-500">({{ $product['reviews'] }})</span>
                                    </div>
                                @endif
                                <div class="mt-3 flex items-center justify-between">
                                    <div class="flex items-baseline gap-2">
                                        <span class="text-base font-extrabold text-zinc-900"><x-currency :amount="number_format($product['price'], 2)" /></span>
                                        @if(!empty($product['old_price']))
                                            <span class="text-sm text-zinc-400 line-through"><x-currency :amount="number_format($product['old_price'], 2)" /></span>
                                        @endif
                                    </div>
